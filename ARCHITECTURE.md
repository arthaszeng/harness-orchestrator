...
**`.cursor/worktrees.json`** — Cursor-native configuration file generated by `harness init` (via `generate_native_artifacts()`; idempotent: skipped if file exists unless `force`). Harness only generates this file; the actual symlink creation is handled entirely by Cursor when it creates a new git worktree.
...
- `worktrees.json` — Cursor-native configuration file generated by `harness init`. Harness only produces this file; Cursor handles symlink creation when creating git worktrees.
