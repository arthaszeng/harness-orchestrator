...
| `harness git-post-ship [--json]` | 发布后清理 |
| **质量门禁** | |
| `harness gate [--task]` | 检查发布门禁 |
| `harness plan-lint --task <id> [--json]` | 验证 plan.md 结构 |
| `harness validate-artifacts --task <id> [--json]` | 产物依赖状态报告 |
| `harness preflight-bundle --task <id> [--json]` | 4 合 1 预检 |
| **产物持久化** | |
| `harness save-eval --task <id> [--kind] [--verdict] ...` | 保存评审结果 |
| `harness save-build-log --task <id> [--body]` | 保存构建日志 |
| `harness save-ship-metrics --task <id> [--body]` | 保存交付指标 |
| `harness save-feedback-ledger --task <id> [--body]` | 保存反馈账本 |
| `harness save-intervention-audit --task <id> ...` | 保存干预审计事件 |
| `harness save-failure --task <id> [--pattern] ...` | 记录失败模式 |
| **评审与校准** | |
| `harness escalation-score compute [--phase] ...` | 计算 escalation 分数 |
| `harness review-score compute [--kind] [--json]` | 校准评审分数与裁定 |
| `harness calibrate [--task] [--json]` | 评审校准报告 |
| `harness trust [--json]` | 渐进式信任档案 |
| `harness record-outcome --task <id> ...` | 记录实际 CI/回滚结果 |
| **工作流辅助** | |
| `harness workflow next [--task] [--json]` | 基于任务状态的工作流提示 |
| `harness task next-id [--json]` | 下一个可用任务 ID |
| `harness search-failures --query <q> [--limit]` | 跨任务搜索失败模式 |
| `harness context-budget --task <id> [--json]` | 估算 token 用量 |
| `harness plan-completion-audit --task <id> [--json]` | 审计交付物完成度 |
| `harness diff-stat [--json]` | 分支 diff 统计 |
| `harness ship-prepare --task <id> [--json]` | 预计算交付元数据 |
| **跨阶段协调** | |
| `harness handoff read\|write [--task] ...` | 结构化跨阶段交接 |
| `harness session read\|write [--task] ...` | 阶段内会话上下文 |
| `harness barrier register\|complete\|check\|list ...` | 异步任务屏障管理 |
| **基础设施** | |
| `harness worktree-setup` | 在 linked worktree 中创建 symlink |
| `harness pr-status [--json]` | 查询 PR 的 CI 和合并状态 |
| `harness ci-logs [--json]` | 获取失败 CI 任务日志 |
