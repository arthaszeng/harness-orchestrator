...
class TestTaskNextId:
    def test_empty_tasks_dir(self, tmp_path, monkeypatch):
        ...
        out = _capture(lambda: run_task_next_id(as_json=True))
        data = json.loads(out)
    ...
    def test_plain_text_output(self, tmp_path, monkeypatch):
        tasks = tmp_path / ".harness-flow" / "tasks"
        tasks.mkdir(parents=True)
        (tasks / "task-003").mkdir()
        monkeypatch.chdir(tmp_path)
        out = _capture(lambda: run_task_next_id(as_json=False))
        assert out.strip() == "task-004"

    def test_no_harness_flow_dir(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        out = _capture(lambda: run_task_next_id(as_json=True))
        data = json.loads(out)
        assert data["next_id"] == "task-001"
...
class TestTaskResolve:
    def test_resolves_latest_task(self, tmp_path, monkeypatch):
        tasks = tmp_path / ".harness-flow" / "tasks"
        ...
        monkeypatch.chdir(tmp_path)
        out = _capture(lambda: run_task_resolve(as_json=True))
        data = json.loads(out)
    ...
    def test_no_task_exits_1(self, tmp_path, monkeypatch):
        (tmp_path / ".harness-flow" / "tasks").mkdir(parents=True)
        monkeypatch.chdir(tmp_path)
    ...
    def test_explicit_task(self, tmp_path, monkeypatch):
        tasks = tmp_path / ".harness-flow" / "tasks"
        (tasks / "task-001").mkdir(parents=True)
        (tasks / "task-002").mkdir()
        monkeypatch.chdir(tmp_path)
        out = _capture(lambda: run_task_resolve(task="task-001", as_json=True))
        data = json.loads(out)
    ...
    def test_phase_unknown_when_no_workflow_state(self, tmp_path, monkeypatch):
        tasks = tmp_path / ".harness-flow" / "tasks"
        (tasks / "task-001").mkdir(parents=True)
        monkeypatch.chdir(tmp_path)
        out = _capture(lambda: run_task_resolve(as_json=True))
